...
const PlanQuiz = () => {
  ...
  const calculateRecommendation = () => {
    ...
    const upgradeSuggestion = suggestUpgrade(
      selectedOptions,
      recommendedPlanId
    );
  ...
  const handleUpgrade = () => {
    if (upgradeRecommendation) {
      const upgradedPlan =
        plans.find((p) => p.id === upgradeRecommendation.upgrade) || null;
    ...
    return (
      ...
      >
        <div className="flex justify-between items-center mb-6">
          ...
          <Link to="/">
            <img src={logo} alt="Nuvme Logo" className="w-40" />
          </Link>
        </div>

        <Card
          className="border-t-4"
          style={{ borderTopColor: finalPlan.color }}
        >
          <CardHeader>
            ...
            {upgradeRecommendation && (
              <div className="p-4 border border-orange-200 bg-orange-50 rounded-lg">
                ...
                >
                  Verificar plano{" "}
                  {
                    plans.find((p) => p.id === upgradeRecommendation.upgrade)
                      ?.name
                  }
                </Button>
              </div>
            ...
            >
              <TabsList className="grid grid-cols-3 w-full">
                {plans.map((plan) => (
                  <TabsTrigger
                    ...
                    value={plan.id}
                    className={`${
                      plan.id === finalPlan.id ? "font-medium" : ""
                    }`}
                  >
                    <div className="flex items-center gap-1">
                      ...
                      {plan.id === finalPlan.id && (
                        <Badge
                          variant="outline"
                          className="ml-1 px-1 py-0 text-xs bg-primary/10"
                        >
                          Recomendado
                        </Badge>
                      )}
              ...
              {plans.map((plan) => (
                <TabsContent key={plan.id} value={plan.id} className="pt-4">
                  <Card>
                    <CardHeader
                      className={`${plan.tailwindColor} ${plan.textColor} rounded-t-lg`}
                    >
                    ...
                    <CardContent className="pt-6 pb-4 space-y-6">
                      ...
                      <div>
                        <h3 className="text-lg font-medium flex items-center gap-2 mb-3">
                          <Icon name="X" className="w-5 h-5 text-red-500" />O
                        ...
                        <ul className="space-y-2 ml-7">
                          {plan.notIncluded.map((item, idx) => (
                            <li
                              ...
                              className="flex items-start gap-2 text-sm"
                            >
                              <span className="text-red-500 font-bold inline-block mt-0.5">
                                ✗
                              </span>
                              <span>{item}</span>
                            </li>
                        ...
                        <ul className="space-y-2 ml-7">
                          {plan.forWho.map((item, idx) => (
                            <li
                              ...
                              className="flex items-start gap-2 text-sm"
                            >
                              <span className="text-blue-500 font-bold inline-block mt-0.5">
                                •
                              </span>
                              <span>{item}</span>
                            </li>
                    ...
                    {Object.entries(selectedOptions).map(
                      ([questionId, optionId]) => {
                        ...
                        return (
                          <div
                            key={questionId}
                            className="pl-7 border-l-2 border-indigo-100"
                          >
  ...
  return (
    ...
    >
      <div className="flex justify-center mb-6">
        <Link to="/">
          <img src={logo} alt="Nuvme Logo" className="w-40 mb-4" />
        </Link>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Responda algumas perguntas para descobrirmos o plano ideal para sua
            empresa
          </CardDescription>
